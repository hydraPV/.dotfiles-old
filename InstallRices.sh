...
case "$MainPrompt" in
  y)
    ...
    if [ -d "$HOME/.dotfiles/" ]; then
      ...
      mv $HOME/.dotfiles/bspwm/ $HOME/.dotfiles
      if [ -d "$HOME/.dotfiles/MOTD" ] && [ -d "$HOME/.dotfiles/Nord/" ]; then
    ...
    while ! [[ "$RicePrompt" =~ ^[1-3]$ ]]; do
      read -p "What Rice do you want to use at first? [Nord '1' MOTD '2']: " RicePrompt
      if ! [[ "$RicePrompt" =~ ^[1-2]$ ]]; then
